...
- `json/horarios/` — Per-period snapshots (`semestre1|2`, `vacaciones1|2` + merged `semestre`/`vacaciones`). Each entry has `tipoPeriodo` (par/impar), `catedratico` and `auxiliar` normalized (spaces collapsed).
- `json/horarios/history/` — Archived raw snapshots per academic cycle (`<ciclo>_<tipoPeriodo>.json`), written by the catalog pipeline.
- `json/catalogo.json` — **Catálogo unificado generado** por `scraper-catalogo.mjs`. Organiza los datos dispersos en: `ciclosAcademicos` (ids generados `ciclo-AAAA-N`, sin id oficial), `tipoPeriodos` (`semestre-impar|par`, `vacaciones-impar|par`), `carreras` (id, nombre, pensums, colores), `cursos` (por código: `observaciones` de **solo "abrió o no"** por `(ciclo, tipoPeriodo)` y `resumen` de apertura por tipoPeriodo), `docentes` (catedráticos **y** auxiliares, con `rol`, `variantes`, `activo`, `ultimoCicloVisto`, `ciclos`, carreras y cursos). Los **traslapes no se calculan aquí** (obedecen reglas específicas que se manejan aparte). La depuración marca `activo:false` a un docente que lleva `DEPURAR_TRAS_CICLOS` (3) ciclos sin aparecer. Umbral configurable al tope del script (`DEPURAR_TRAS_CICLOS`).
