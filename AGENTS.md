...
- **Routing:** React Router, 6 routes: `/` → Home, `/visualizador` → Visualizer, `/foro` → Forum, `/grupos` → WhatsAppGroups, `/mis-publicaciones` → MyPosts, `/notificaciones` → Notifications. Wrapped in `HelmetProvider` (SEO via `src/components/seo/Seo.jsx`), `ThemeProvider`, `NotificationsProvider`, `OnboardingProvider`.
- **Core JS modules** (not React components) live in `src/modules/`:
  ...
  - `data/plannerWarnings.js` — Avisos del planificador por curso/bloque: apertura en el último ciclo del periodo, traslapes de la misma sección (reusa `calcularTraslapeMinutos`/`esTraslapePermitido` de `scraper.js`), reputación de catedráticos y límite de horas magistrales en vacaciones (máx 4h/día). Usado por `Planner.jsx` (git log: "Planificador ahora responde al sistema de recomendaciones y muestra advertencias sobre los cursos").
...
- **React pages:** `src/pages/Home.jsx` (landing; las CareerCards leen `pensums`/`carreras[].colores` del catálogo, sin fetch a `index.json` ni `pensum_color/`), `src/pages/Visualizer.jsx` (graph viewer + toolbar + planner + schedule), plus community pages `Forum.jsx` (~1900 lines), `WhatsAppGroups.jsx`, `MyPosts.jsx`, `Notifications.jsx`.
- **React components:** `src/components/Navbar.jsx`, planner components `Planner.jsx`, `CoursePool.jsx`, `SemesterBlock.jsx`, `VacationBlock.jsx`, `CourseChip.jsx`, `ToastNotification.jsx`, `ExportModal.jsx`, `DocenteReviews.jsx`, shared UI primitives in `components/ui/`, `ScheduleBuilder.jsx` (~2000 lines, the schedule view), and onboarding components in `components/onboarding/`.
