...
export default function Visualizer() {
    ...
    useEffect(() => {
        const handleThemeChange = () => {
            ...
            if (graphManager) {
                if (typeof graphManager.setTemaOscuro === 'function') graphManager.setTemaOscuro(isDark);
                else graphManager.temaOscuro = isDark;
    ...
    const handleLimpiar = () => {
        ...
        graphManager.desseleccionarNodo();
        if (graphManager.storageManager) {
            graphManager.storageManager.limpiarProgreso(graphManager.cursos);
            actualizarCreditos();
            graphManager.dibujarGrafo();
        }
    ...
    const handleCerrarInfo = () => {
        if (graphManager) graphManager.desseleccionarNodo();
        setSelectedCourse(null);
    };
    ...
    const handleToggleOptativos = () => {
        if (graphManager) {
            ...
            graphManager.setShowOptional(newValue);
        }
    };
    ...
    const handleToggleRutaCritica = () => {
        if (graphManager) {
            ...
            graphManager.setShowCriticalPath(newValue);
        }
    };
    ...
    const handleCambiarVista = () => {
        if (graphManager) {
            ...
            setViewMode(newMode);
            graphManager.setViewMode(newMode);
        }
    };
    ...
    const handleLayoutChange = async () => {
        ...
        setLayout(newLayout);
        if (graphManager) {
            graphManager.setCurrentLayout(newLayout);
            await graphManager.dibujarGrafo();
            if (panZoom) panZoom.zoomReset();
        }
    };

    const handlePensumChange = async (e) => {
        const relPath = e.target.value;
        if (!relPath || !graphManager) return;
        try {
            await loadPensum(relPath);
            if (graphManager.storageManager) {
                graphManager.storageManager.cargarProgreso(cursos, cursoMap);
            }
            actualizarCreditos();
            graphManager.updateCursos(cursos, cursoMap);
            await graphManager.dibujarGrafo();
            setSelectedCourse(null);
        } catch (error) {
            console.error(error);
    ...
    const handleToggleCompletado = (cursoId) => {
        if (!graphManager || !graphManager.storageManager) return;
        
        graphManager.storageManager.toggleCompletado(cursoId, graphManager, { 
            mostrar: () => {} 
        });
    ...
    const handleZoomReset = () => { if (panZoom) panZoom.zoomReset(); };
    ...
    const handleSelectSearch = (curso) => {
        ...
        setSearchResults([]);
        if (graphManager) {
            graphManager.seleccionarNodo(curso, document.getElementById('grafica-group'));
        ...
        return ids.map(id => {
            ...
            const handleBadgeLeave = (e) => {
                if (!isDarkMode) {
                    ...
                    e.currentTarget.style.backgroundColor = bgStatus;
                }
            ...
            return (
                <span 
                    key={id} 
                    className="px-[6px] py-[2px] rounded-[4px] text-[0.75rem] font-bold cursor-help border transition-colors" 
    ...
    return (
        <div className="flex-1 flex flex-col w-full h-full overflow-hidden bg-[#FAFBFC] dark:bg-[#121924] text-[#172B4D] dark:text-slate-100 font-sans transition-colors duration-300">
            ...
            <div className="flex flex-col md:flex-row items-center justify-between p-3 border-b border-[#DFE1E6] dark:border-[#3E4C5E] bg-white dark:bg-[#1C2636] shadow-sm z-20 shrink-0 gap-3">
                
                <div className="flex items-center space-x-2 bg-black/5 dark:bg-white/5 p-1 rounded-lg w-full md:w-auto overflow-x-auto hide-scrollbar">
                    <button onClick={handleCambiarVista} className={`flex items-center space-x-1.5 px-3 py-1.5 text-xs font-bold rounded-md transition border-none cursor-pointer whitespace-nowrap ${viewMode === 'semester' ? (isDarkMode ? 'bg-[#3E4C5E] text-white' : 'bg-white text-[#0052CC] shadow-sm') : 'text-current hover:bg-[#F4F5F7] dark:hover:bg-[#3E4C5E] bg-transparent'}`}>
                        <Grid size={14} /> <span>Semestral</span>
                    </button>
                    <button onClick={handleToggleRutaCritica} className={`flex items-center space-x-1.5 px-3 py-1.5 text-xs font-bold rounded-md transition border-none cursor-pointer whitespace-nowrap ${showCriticalPath ? (isDarkMode ? 'bg-[#3E4C5E] text-white' : 'bg-white text-[#0052CC] shadow-sm') : 'text-current hover:bg-[#F4F5F7] dark:hover:bg-[#3E4C5E] bg-transparent'}`}>
                        <Compass size={14} /> <span>Ruta Crítica</span>
                    </button>
                    <button onClick={handleToggleOptativos} className={`flex items-center space-x-1.5 px-3 py-1.5 text-xs font-bold rounded-md transition border-none cursor-pointer whitespace-nowrap ${showOptional ? (isDarkMode ? 'bg-[#3E4C5E] text-white' : 'bg-white text-[#0052CC] shadow-sm') : 'text-current hover:bg-[#F4F5F7] dark:hover:bg-[#3E4C5E] bg-transparent'}`}>
                        <Layers size={14} /> <span>Optativos</span>
                    </button>
                    <button onClick={handleLayoutChange} className={`flex items-center space-x-1.5 px-3 py-1.5 text-xs font-bold rounded-md transition border-none cursor-pointer whitespace-nowrap text-current hover:bg-[#F4F5F7] dark:hover:bg-[#3E4C5E] bg-transparent`}>
                        <LayoutTemplate size={14} /> <span>{layout === 'vertical' ? 'Vertical' : 'Horizontal'}</span>
                    </button>
                </div>

                <div className="flex flex-wrap items-center justify-center md:justify-end gap-2 md:gap-3 w-full md:w-auto shrink-0">
                    <button
                        type="button"
                        onClick={() => setShowGuia(true)}
                        className="w-8 h-8 rounded-full border border-[#DFE1E6] dark:border-[#3E4C5E] bg-white dark:bg-[#1C2636] text-[#0052CC] dark:text-[#4C9AFF] font-extrabold text-sm flex items-center justify-center hover:bg-[#F4F5F7] dark:hover:bg-[#2D333B] transition"
                        aria-label="Ayuda"
                        title="Ayuda"
                    >
                        ?
                    ...
                    <select
                        ...
                        className="bg-[#FAFBFC] dark:bg-[#0E1624] border border-[#DFE1E6] dark:border-[#3E4C5E] text-[#172B4D] dark:text-white rounded px-2.5 py-1.5 text-xs focus:outline-none focus:border-[#0052CC] dark:focus:border-[#4C9AFF] cursor-pointer max-w-[130px] sm:max-w-none truncate"
                    >
                    ...
                    <div className="relative">
                        ...
                        <input
                            ...
                            className="bg-[#FAFBFC] dark:bg-[#0E1624] border border-[#DFE1E6] dark:border-[#3E4C5E] text-[#172B4D] dark:text-white rounded px-2.5 py-1 pl-8 text-xs focus:outline-none focus:border-[#0052CC] dark:focus:border-[#4C9AFF] w-32 sm:w-48 transition-all"
                        />
                        {searchResults.length > 0 && (
                            <div className="absolute top-[100%] right-0 sm:left-0 w-48 mt-[4px] shadow-lg rounded-md overflow-hidden z-[5000] bg-white dark:bg-[#1C2636] border border-[#DFE1E6] dark:border-[#3E4C5E]">
                    ...
                    <div className="flex items-center space-x-1.5 bg-[#DEEBFF] dark:bg-[#0C295E] px-2 py-1 rounded border border-[#0052CC]/20 dark:border-[#4C9AFF]/20 shadow-sm shrink-0">
                        <span className="text-[10px] font-bold text-[#0052CC] dark:text-[#4C9AFF] uppercase tracking-wider hidden sm:inline">Créditos:</span>
                        <span className="text-xs font-extrabold text-[#0052CC] dark:text-[#4C9AFF]">{creditosAprobados}</span>
                    </div>

                    <button onClick={handleLimpiar} className="flex items-center justify-center p-1.5 rounded bg-red-50 hover:bg-red-100 dark:bg-red-900/30 dark:hover:bg-red-900/50 text-[#BF2600] dark:text-red-400 transition border-none cursor-pointer" title="Limpiar Selección">
                        <RotateCcw size={14} />
                    </button>
                </div>
            </div>

            <div className={`flex-1 relative overflow-hidden contenedor-grafica transition-colors duration-300 ${isDarkMode ? 'bg-[#0E1624] tema-oscuro' : 'bg-[#FAFBFC]'}`} ref={graficaRef} id="grafo-canvas"></div>

            <div className="flex justify-center items-center gap-[6px] absolute bottom-[24px] right-[24px] z-[2100] select-none">
            ...
            {selectedCourse && (
                <div className={`absolute top-[80px] right-[20px] w-[340px] max-md:top-auto max-md:bottom-0 max-md:left-0 max-md:right-0 max-md:w-full max-md:rounded-b-none max-md:rounded-t-[15px] max-md:max-h-[50vh] max-md:overflow-y-auto backdrop-blur-md rounded-[12px] shadow-xl p-[20px] z-[950] border fade-in max-md:p-[15px] select-none ${isDarkMode ? 'bg-[#1C2636]/95 border-[#3E4C5E] text-slate-100' : 'bg-white/95 border-[#DFE1E6] text-[#172B4D]'}`}>
